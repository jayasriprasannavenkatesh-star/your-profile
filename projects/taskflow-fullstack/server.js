...
const app = express();
const dataDir = process.env.DB_PATH ? path.dirname(path.resolve(process.env.DB_PATH)) : path.join(__dirname, 'data');
const dbPath = process.env.DB_PATH ? path.resolve(process.env.DB_PATH) : path.join(dataDir, 'taskflow.db');
...
app.get('/health', (req,res) => res.json({ status:'ok', service:'taskflow-api' }));
